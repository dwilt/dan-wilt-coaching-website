...
add_action( 'after_setup_theme', 'jsd_theme_setup' );
...
function jsd_tiny_mce_edits( $args ) {
...
add_filter( 'tiny_mce_before_init', 'jsd_tiny_mce_edits' );
...
function jsd_toolbars( $toolbars ){
...
add_filter( 'acf/fields/wysiwyg/toolbars' , 'jsd_toolbars'  );

// For the FIRST row of the Full WYSIWYG buttons
function jsd_tinymce_buttons($buttons) {
...
add_filter('mce_buttons','jsd_tinymce_buttons');

// For the SECOND row of the Full WYSIWYG buttons
function jsd_tinymce_buttons2($buttons) {
	//echo "<pre>"; print_r($buttons); echo "</pre>";
	/*
	Array (
	    [0] => formatselect
...
add_filter('mce_buttons_2','jsd_tinymce_buttons2');
...
function jsd_add_excerpts_to_pages() {
     add_post_type_support( 'page', 'excerpt' );
}
add_action( 'init', 'jsd_add_excerpts_to_pages' );
